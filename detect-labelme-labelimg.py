...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    # 遍历特定文件夹
    imgDir = "/home/chenpengfei/dataset/frame-16/train"
    i = 0
    totalTime = 0.0
    t0 = time.time()
                ...
                for dir1 in dirs1:
                    ...
                    for root2, dirs2, files2 in os.walk(imgDir2):
                        for dir2 in dirs2:
                            ...
                            # for file in os.listdir(imgDir3):
                            #     if (file.split(".")[-1] in ["jpg", "png"]):
                            #       i += 1
                            #       imagefile = os.path.join(imgDir3, file)
                            #       print(imagefile)

                            Files = os.listdir(imgDir3)
                            ...
                            for path, img, im0s, vid_cap in dataset:
                                ...
                                if pt:
                                    ...
                                    # torch.save
                                    # torch.save(model, "./weights/yolov5s_all.pt")
                                    # #export torchscript
                                    # script_module = torch.jit.trace(model, img, strict=False)
                                    # torch.jit.save(script_module, "./weights/yolov5s.torchscript.pt")
                                    # break
                                elif onnx:
                                ...
                                for i, det in enumerate(pred):  # detections per image
                                    ...
                                    if len(det):
                                        ...
                                        for *xyxy, conf, cls in reversed(det):
                                            ...
                                            if save_labelimg_txt:
                                                xyxy = (torch.tensor(xyxy).view(1, 4)).view(-1).tolist()  # normalized xywh
                                                if (cls.cpu().numpy() == 0):
                                                    pass
                                                elif (cls.cpu().numpy() == 1):
                                                    txt_path = path.split(".")[0]
                                                    with open(txt_path + '.txt', 'a') as f:
                                                        line = str(xyxy[0]) + " " + str(xyxy[1]) + " " + str(xyxy[2]) + " " + str(xyxy[3]) + " 2\n"
                                                        f.write(line)
                                            if save_json:  # Write to json
                                                xyxy = (torch.tensor(xyxy).view(1, 4)).view(-1).tolist()  # normalized xywh
                                                object = {}
                                                if (cls.cpu().numpy() == 0):
                                                    object["label"] = "hand"
                                    ...
                                    print(f'{s}inference + NMS: ({(t2 - t1) * 1000.0:.3f}ms)')
    ...
    print(f'Done. ({(time.time() - t0) * 1000.0:.3f}ms)')


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='./weights/yolov5s.onnx', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='data/CocoTest100', help='file/dir/URL/glob, 0 for webcam')
